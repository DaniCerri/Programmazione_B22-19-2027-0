...
Nel linguaggio comune siamo abituati a risolvere problemi ogni giorno senza pensare in modo analitico alle singole azioni che compiamo. Tuttavia, quando vogliamo delegare la risoluzione di un problema a un calcolatore (o spiegare a un'altra persona come agire senza margini d'errore), abbiamo bisogno di un metodo rigoroso.

\begin{concetto}[Definizione di Algoritmo]
Un \textbf{algoritmo} è una sequenza finita e ordinata di istruzioni elementari, chiare e non ambigue, la cui esecuzione porta alla risoluzione di un determinato problema in un tempo finito.
\end{concetto}
...
\begin{enumerate}
  ...
  \item \textbf{Definitezza (Non ambiguità):} Ogni istruzione deve essere univoca e comprensibile senza alcuna interpretazione soggettiva. Frasi come ``mescola un po'\,'' o ``aspetta qualche secondo'' sono inaccettabili in un algoritmo informatico: bisogna specificare ``mescola per 3 minuti a velocità 2'' oppure ``attendi 5 secondi''.
  
  \item \textbf{Efficienza:} L'algoritmo deve risolvere il compito impiegando una quantità ragionevole di tempo e di memoria. 
  \begin{consiglio}[Nota Didattica per la Prima Annualità]
  Nel corso di quest'anno la priorità assoluta sarà comprendere la \textbf{logica} e raggiungere la \textbf{correttezza}; la misurazione scientifica dell'efficienza (complessità computazionale) sarà affrontata negli anni successivi.
  \end{consiglio}
...
Mettiamoci alla prova formulando un algoritmo esatto per un problema che affrontiamo ogni giorno.

\begin{esercizio}[Attraversare la strada in presenza di un semaforo]
...
% ------------------------------------------------------------------------------
\section{Verso le Prossime Lezioni}
% ------------------------------------------------------------------------------

Nelle prossime lezioni trasformeremo questo modo di pensare in due linguaggi formali utilizzati universalmente dai programmatori di tutto il mondo:
\begin{itemize}
  \item I \textbf{Diagrammi di Flusso (Flowchart):} La rappresentazione grafica degli algoritmi tramite blocchi geometrici (rettangoli per le azioni, rombi per le condizioni, frecce per la direzione).
  \item Lo \textbf{Pseudocodice:} Un linguaggio a metà strada tra la lingua naturale e i linguaggi di programmazione reali (come Python e C), che ci permetterà di scrivere logiche complesse prima di passare alla tastiera del computer.
\end{itemize}
